		...
		case 3:
			...
			while(1) {
				...
				while(z<1 || z > Dfim){
					...
					scanf("%d", &z);
					if(z == qCartas){
						//consertando idiotices
					} else if(deck[z] % 2 == 0 || z > qCartas || deck[z] == 57 || deck[z] == 58 || deck[z] == 75 || deck[z] == 76){ 	//FILTRO PARA MALDIÇÕES!!!!!!
...
void mercante() {
	...
	if(rando == 3) {
		...
		printf("\t\tBem-vindo à loja, aventureiro\n");
	}	
	printf("\n");
...
void venda() {
	...
	SetConsoleTextAttribute(h,14);
		printf("\n\tDIGITE O Nº DA CARTA QUE DESEJA COMPRAR");
